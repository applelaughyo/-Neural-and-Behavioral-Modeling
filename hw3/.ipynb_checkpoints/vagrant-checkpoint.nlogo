...
to setup-turtles
  ...
  create-turtles 10    ;; uses the value of the number slider to create turtles
...
to reproduce
  ask turtles [
    if energy > 100 [
      set energy energy - 50  ;; take away birth-energy to give birth
      hatch 1 [ set energy 10 ] ;; give this birth-energy to the offspring
    ]
  ]
end
...
to regrow-grass
  ask patches [ ;; k out of 100 times, the patch color is set to green
    if random 100 < 0 [ set pcolor white ]
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
...
"grass" 1.0 0 -11085214 true "" "plot count patches with [pcolor = green]"
